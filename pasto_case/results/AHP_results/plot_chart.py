...
def plot_radar_chart(data, metrics, title, yticks_value, yticks_name):
    ...
    labels = ['VCI', 'VE-L1', 'VE-L2', 'VE-L3', 'VGN', 'VHEE-L1', 'VHEE-L2', 'VHEE-L3']
    
    for idx, (i, row) in enumerate(data.iterrows()):
        values = row[metrics].values.flatten().tolist()
        values = values + [values[0]]
        ax.plot(theta, values, linewidth=1.0, linestyle='solid', label=labels[i], marker='o', markersize=6, color=color_palette[idx % len(color_palette)])
    metrics = ['Factor de disponibilidad', 'Rango de conducción', 'Costo acumulado', 'Incentivos', 'Emisiones']
...
def plot_radar_chart2(data, metrics, title, yticks_value, yticks_name):
    N = len(metrics)
    theta = np.linspace(0, 2 * np.pi, N, endpoint=False)
    theta = np.concatenate([theta, [theta[0]]])
    ...
    color_palette = ['#a4165f', '#d54855', '#f4814b', '#ffbc4f']
...
plt.savefig("C:/Users/noluc/OneDrive/Escritorio/index1.jpg")
plt.show()

yticks_value = [0, 0.05, 0.10, 0.15, 0.20]
yticks_name = ["0", "0.05", "0.10", "0.15", "0.20"]
fig = plot_radar_chart(df2, metrics, '', yticks_value, yticks_name)
plt.savefig("C:/Users/noluc/OneDrive/Escritorio/index2.jpg")
plt.show()
...
plt.savefig("C:/Users/noluc/OneDrive/Escritorio/index3.jpg")
plt.show()

yticks_value = [0, 0.02, 0.05, 0.07, 0.09]
yticks_name = ["0", "0.02", "0.05", "0.07", "0.09"]
fig = plot_radar_chart(df4, metrics, '', yticks_value, yticks_name)
plt.savefig("C:/Users/noluc/OneDrive/Escritorio/index4.jpg")
plt.show()

yticks_value = [0, 0.02, 0.05, 0.07, 0.09]
yticks_name = ["0", "0.02", "0.05", "0.07", "0.09"]
fig = plot_radar_chart2(df5, metrics, '', yticks_value, yticks_name)
plt.savefig("C:/Users/noluc/OneDrive/Escritorio/index5.jpg")
plt.show()


# data_frame1 = pd.read_csv("C:/Nohora/UniValle_project/pasto_case/results/AHP_results/AHP_TOPSIS_results1.csv")
# df1 = data_frame1.copy()
# df1 = df1.drop(['Availability Factor','Driving Range','Accumulated Cost','Incentives','Emissions'], axis=1)
# metrics = df1.columns[1:].tolist()

# data_frame2 = pd.read_csv("C:/Nohora/UniValle_project/pasto_case/results/AHP_results/AHP_TOPSIS_results2.csv")
# df2 = data_frame2.copy()
# df2 = df2.drop(['Availability Factor','Driving Range','Accumulated Cost','Incentives','Emissions'], axis=1)

# data_frame3 = pd.read_csv("C:/Nohora/UniValle_project/pasto_case/results/AHP_results/AHP_TOPSIS_results3.csv")
# df3 = data_frame3.copy()
# df3 = df3.drop(['Availability Factor','Driving Range','Accumulated Cost','Incentives','Emissions'], axis=1)

# data_frame4 = pd.read_csv("C:/Nohora/UniValle_project/pasto_case/results/AHP_results/AHP_TOPSIS_results4.csv")
# df4 = data_frame4.copy()
# df4 = df4.drop(['Availability Factor','Driving Range','Accumulated Cost','Incentives','Emissions'], axis=1)
