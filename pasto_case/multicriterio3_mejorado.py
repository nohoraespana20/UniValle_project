...
def power_generated(area_available):
    ...
    areaPanelPV = 3
    hps = 3.5
    numberPanelPV = math.floor(area_available / areaPanelPV)
...
def run_doper_model(demand_scale=None, pv_scale=None, battery_capacities=None):
    ...
    # print(df[["Import Power [kW]", "Export Power [kW]", "PV Power [kW]", "Battery Charging Power [kW]", "Battery Discharging Power [kW]"]])
...
def calculate_iip(E_export, year, E_pv):
    ...
    if E_pv == 0 or E_pv is None:
        # print(f"WARNING: E_pv es cero o None. E_export={E_export}, E_pv={E_pv}")
        return 0 
...
class EVChargingYearlyProblem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        P_bat_total = sizing_battery_system(P_pv)  # Ahora basado en área, no en P_pv
        
        # Distribuir capacidades de batería por nodo (mismas proporciones que PV)
        capacidades = [P_bat_total*0.15, P_bat_total*0.13, P_bat_total*0.72]
...
carpeta_salida = "C:/Nohora/UniValle_project/pasto_case/opt_mejorado_3"
os.makedirs(carpeta_salida, exist_ok=True)

archivos_csv = [os.path.join(carpeta_salida, f"solucion_{i+1}.csv") for i in range(5)]
columnas = ["Año", "EV", "PHEV", "tL2", "tL3", "area",
