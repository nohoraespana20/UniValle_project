...
def parameters():
    ...
    if False:
        ...
        # #LEVEL CHARGE : L3
        parameter['tariff']['energy'] = {0:0.75, 1:0.75, 2:1.5} # $/kWh for periods 0-offpeak, 1-midpeak, 2-onpeak
        parameter['tariff']['demand'] = {0:0.75, 1:0.75, 2:1.5} # $/kW for periods 0-offpeak, 1-midpeak, 2-onpeak
        # parameter['tariff']['demand'] = {0:67.9, 1:67.9, 2:67.9} # $/kW for periods 0-offpeak, 1-midpeak, 2-onpeak
    ...
    parameter['site']['import_max'] = 100000000 # kW
    parameter['site']['export_max'] = 100000000 # kW
    ...
    parameter['network']['nodes'] = [ # list of dict to define inputs for each node in network
        ...
        { # node 29 Corponariño
            ...
            'pcc': True,
            'slack': False, 
            'load_id': 'pf_demand_node29',
            'ders': { 
                'pv_id': 'pf_pv_node29',
                'pv_maxS': 199,
                'battery': 'pf_bat_node29', # node can contain multiple battery assets, so should be list
                'genset': None,
                'load_control': None # node likely to only contain single load_control asset, so should be str
            },
            'connections': [
