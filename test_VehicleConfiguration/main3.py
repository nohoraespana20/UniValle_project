...
if __name__ == '__main__':
    ...
    for c in range(min_speed, max_speed, steps):
        ...
        while traci.simulation.getMinExpectedNumber() > 0:
            ...
            vehicleData = traci.vehicle.getHCEmission(vehicles[0])
...
###########################################
    ...
    plt.title("vClass=bus / emissionClass=HBEFA3/PC_G_EU2")
    plt.xlabel("Speed [km/h]")
    plt.ylabel("HC Emissions [g]")
    plt.grid()
    plt.savefig('figures/EURO2_HC_9.png')
    plt.show()
    ...
    plt.ylabel("g/km")
    plt.grid()
    plt.savefig('figures/EURO2_g_km_9.png')
    plt.show()
